# -*- coding: utf-8 -*-

# Form implementation generated from reading ui file 'editar_Adm.ui'
#
# Created by: PyQt5 UI code generator 5.15.6
#
# WARNING: Any manual changes made to this file will be lost when pyuic5 is
# run again.  Do not edit this file unless you know what you are doing.


from PyQt5 import QtCore, QtGui, QtWidgets


class Ui_editar_Adm(object):
    def setupUi(self, editar_Adm):
        editar_Adm.setObjectName("editar_Adm")
        editar_Adm.resize(800, 600)
        editar_Adm.setMinimumSize(QtCore.QSize(800, 600))
        editar_Adm.setMaximumSize(QtCore.QSize(800, 600))
        icon = QtGui.QIcon()
        icon.addPixmap(QtGui.QPixmap("../../Downloads/Icone-_1_.ico"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        editar_Adm.setWindowIcon(icon)
        editar_Adm.setStyleSheet("#editar_Adm{\n"
"    \n"
"    \n"
"    background-image:url(/home/pi/Documents/projeto tcc/ZSQ/ZSQ/imagens/ceu.png)\n"
"    \n"
"\n"
"\n"
"}\n"
"\n"
"\n"
"#frame{\n"
"\n"
"    border-radius:10px;\n"
"    \n"
"    background-color:#0055aa;\n"
"    \n"
"    \n"
"\n"
"    \n"
"\n"
"\n"
"}\n"
"\n"
"\n"
"#btn_goCadastro2\n"
"{\n"
"\n"
"    border-radius:15px;\n"
"    \n"
"    background-color: rgb(94, 197, 230);\n"
"    \n"
"    border-style:solid;\n"
"\n"
"    font-weight:bold;\n"
"    color: rgb(255, 255, 255);\n"
"\n"
"}\n"
"\n"
"#lbl_estilo{\n"
"background-color: rgb(255, 255, 255);\n"
"\n"
"\n"
"}\n"
"\n"
"#spinExerrciciosSemana{\n"
"\n"
"border-radius:10px;\n"
"\n"
"\n"
"}\n"
"\n"
"\n"
"\n"
"")
        self.centralwidget = QtWidgets.QWidget(editar_Adm)
        self.centralwidget.setObjectName("centralwidget")
        self.btn_goCadastro2 = QtWidgets.QPushButton(self.centralwidget)
        self.btn_goCadastro2.setGeometry(QtCore.QRect(360, 500, 81, 41))
        self.btn_goCadastro2.setStyleSheet("")
        self.btn_goCadastro2.setObjectName("btn_goCadastro2")
        self.label = QtWidgets.QLabel(self.centralwidget)
        self.label.setGeometry(QtCore.QRect(230, 50, 331, 20))
        font = QtGui.QFont()
        font.setFamily("Arial Black")
        font.setPointSize(16)
        font.setBold(True)
        font.setWeight(75)
        self.label.setFont(font)
        self.label.setStyleSheet("color: rgb(255, 255, 255);")
        self.label.setAlignment(QtCore.Qt.AlignCenter)
        self.label.setWordWrap(False)
        self.label.setObjectName("label")
        self.lbl_nome_2 = QtWidgets.QLabel(self.centralwidget)
        self.lbl_nome_2.setGeometry(QtCore.QRect(140, 120, 133, 19))
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(12)
        font.setBold(True)
        font.setWeight(75)
        self.lbl_nome_2.setFont(font)
        self.lbl_nome_2.setStyleSheet("color: rgb(255, 255, 255);")
        self.lbl_nome_2.setObjectName("lbl_nome_2")
        self.txt_cpf = QtWidgets.QLineEdit(self.centralwidget)
        self.txt_cpf.setGeometry(QtCore.QRect(140, 150, 133, 20))
        self.txt_cpf.setStyleSheet("background-color: rgb(34, 150, 213);\n"
"border:solid;\n"
"border-radius:30px;\n"
"color:white;")
        self.txt_cpf.setInputMask("000.000.000-00")
        self.txt_cpf.setObjectName("txt_cpf")
        self.lbl_nome = QtWidgets.QLabel(self.centralwidget)
        self.lbl_nome.setGeometry(QtCore.QRect(320, 110, 133, 45))
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(12)
        font.setBold(True)
        font.setWeight(75)
        self.lbl_nome.setFont(font)
        self.lbl_nome.setStyleSheet("color: rgb(255, 255, 255);")
        self.lbl_nome.setObjectName("lbl_nome")
        self.txt_nome = QtWidgets.QLineEdit(self.centralwidget)
        self.txt_nome.setGeometry(QtCore.QRect(320, 150, 133, 20))
        self.txt_nome.setStyleSheet("background-color: rgb(34, 150, 213);\n"
"border:solid;\n"
"color:white;")
        self.txt_nome.setObjectName("txt_nome")
        self.lbl_idade = QtWidgets.QLabel(self.centralwidget)
        self.lbl_idade.setGeometry(QtCore.QRect(490, 120, 133, 31))
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(12)
        font.setBold(True)
        font.setWeight(75)
        self.lbl_idade.setFont(font)
        self.lbl_idade.setStyleSheet("color: rgb(255, 255, 255);")
        self.lbl_idade.setObjectName("lbl_idade")
        self.txt_idade = QtWidgets.QLineEdit(self.centralwidget)
        self.txt_idade.setGeometry(QtCore.QRect(490, 150, 133, 20))
        self.txt_idade.setStyleSheet("background-color: rgb(34, 150, 213);\n"
"border-style:solid;\n"
"border-radius:15px;\n"
"color:white;")
        self.txt_idade.setObjectName("txt_idade")
        self.lbl_nome_3 = QtWidgets.QLabel(self.centralwidget)
        self.lbl_nome_3.setGeometry(QtCore.QRect(140, 200, 133, 19))
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(12)
        font.setBold(True)
        font.setWeight(75)
        self.lbl_nome_3.setFont(font)
        self.lbl_nome_3.setStyleSheet("color: rgb(255, 255, 255);")
        self.lbl_nome_3.setObjectName("lbl_nome_3")
        self.lbl_nome_4 = QtWidgets.QLabel(self.centralwidget)
        self.lbl_nome_4.setGeometry(QtCore.QRect(140, 280, 281, 19))
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(12)
        font.setBold(True)
        font.setWeight(75)
        self.lbl_nome_4.setFont(font)
        self.lbl_nome_4.setStyleSheet("color: rgb(255, 255, 255);")
        self.lbl_nome_4.setObjectName("lbl_nome_4")
        self.spinHorasDormidas = QtWidgets.QSpinBox(self.centralwidget)
        self.spinHorasDormidas.setGeometry(QtCore.QRect(540, 280, 141, 22))
        self.spinHorasDormidas.setStyleSheet("background-color: rgb(34, 150, 213);\n"
"color:white;")
        self.spinHorasDormidas.setObjectName("spinHorasDormidas")
        self.lbl_nome_5 = QtWidgets.QLabel(self.centralwidget)
        self.lbl_nome_5.setGeometry(QtCore.QRect(140, 310, 371, 19))
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(12)
        font.setBold(True)
        font.setWeight(75)
        self.lbl_nome_5.setFont(font)
        self.lbl_nome_5.setStyleSheet("color: rgb(255, 255, 255);")
        self.lbl_nome_5.setObjectName("lbl_nome_5")
        self.spinXicarasCafe = QtWidgets.QSpinBox(self.centralwidget)
        self.spinXicarasCafe.setGeometry(QtCore.QRect(540, 310, 141, 22))
        self.spinXicarasCafe.setStyleSheet("background-color: rgb(34, 150, 213);\n"
"color:white;\n"
"")
        self.spinXicarasCafe.setObjectName("spinXicarasCafe")
        self.lbl_nome_6 = QtWidgets.QLabel(self.centralwidget)
        self.lbl_nome_6.setGeometry(QtCore.QRect(140, 340, 371, 19))
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(12)
        font.setBold(True)
        font.setWeight(75)
        self.lbl_nome_6.setFont(font)
        self.lbl_nome_6.setStyleSheet("color: rgb(255, 255, 255);")
        self.lbl_nome_6.setObjectName("lbl_nome_6")
        self.spinExercicioSemana = QtWidgets.QSpinBox(self.centralwidget)
        self.spinExercicioSemana.setGeometry(QtCore.QRect(540, 340, 141, 22))
        self.spinExercicioSemana.setStyleSheet("background-color: rgb(34, 150, 213);\n"
"color:white;")
        self.spinExercicioSemana.setObjectName("spinExercicioSemana")
        self.lbl_nome_7 = QtWidgets.QLabel(self.centralwidget)
        self.lbl_nome_7.setGeometry(QtCore.QRect(140, 370, 371, 19))
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(12)
        font.setBold(True)
        font.setWeight(75)
        self.lbl_nome_7.setFont(font)
        self.lbl_nome_7.setStyleSheet("color: rgb(255, 255, 255);")
        self.lbl_nome_7.setObjectName("lbl_nome_7")
        self.spinExercicioSemana_2 = QtWidgets.QSpinBox(self.centralwidget)
        self.spinExercicioSemana_2.setGeometry(QtCore.QRect(540, 370, 141, 22))
        self.spinExercicioSemana_2.setStyleSheet("background-color: rgb(34, 150, 213);\n"
"color:white;")
        self.spinExercicioSemana_2.setMaximum(10)
        self.spinExercicioSemana_2.setObjectName("spinExercicioSemana_2")
        self.lbl_nome_9 = QtWidgets.QLabel(self.centralwidget)
        self.lbl_nome_9.setGeometry(QtCore.QRect(310, 200, 161, 19))
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(12)
        font.setBold(True)
        font.setWeight(75)
        self.lbl_nome_9.setFont(font)
        self.lbl_nome_9.setStyleSheet("color: rgb(255, 255, 255);")
        self.lbl_nome_9.setObjectName("lbl_nome_9")
        self.lbl_nome_10 = QtWidgets.QLabel(self.centralwidget)
        self.lbl_nome_10.setGeometry(QtCore.QRect(490, 200, 191, 19))
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(12)
        font.setBold(True)
        font.setWeight(75)
        self.lbl_nome_10.setFont(font)
        self.lbl_nome_10.setStyleSheet("color: rgb(255, 255, 255);")
        self.lbl_nome_10.setObjectName("lbl_nome_10")
        self.frame = QtWidgets.QFrame(self.centralwidget)
        self.frame.setGeometry(QtCore.QRect(80, 30, 641, 541))
        self.frame.setStyleSheet("")
        self.frame.setFrameShape(QtWidgets.QFrame.StyledPanel)
        self.frame.setFrameShadow(QtWidgets.QFrame.Raised)
        self.frame.setObjectName("frame")
        self.Sexo_Feminino_2 = QtWidgets.QRadioButton(self.frame)
        self.Sexo_Feminino_2.setGeometry(QtCore.QRect(70, 220, 82, 17))
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(10)
        font.setBold(False)
        font.setWeight(50)
        self.Sexo_Feminino_2.setFont(font)
        self.Sexo_Feminino_2.setStyleSheet("color: rgb(255, 255, 255);")
        self.Sexo_Feminino_2.setChecked(False)
        self.Sexo_Feminino_2.setAutoRepeat(True)
        self.Sexo_Feminino_2.setAutoExclusive(False)
        self.Sexo_Feminino_2.setObjectName("Sexo_Feminino_2")
        self.Sexo_masculino_2 = QtWidgets.QRadioButton(self.frame)
        self.Sexo_masculino_2.setEnabled(True)
        self.Sexo_masculino_2.setGeometry(QtCore.QRect(70, 200, 82, 17))
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(10)
        self.Sexo_masculino_2.setFont(font)
        self.Sexo_masculino_2.setStyleSheet("color: rgb(255, 255, 255);")
        self.Sexo_masculino_2.setAutoRepeat(True)
        self.Sexo_masculino_2.setAutoExclusive(False)
        self.Sexo_masculino_2.setObjectName("Sexo_masculino_2")
        self.Sono_AgitadoSim_2 = QtWidgets.QRadioButton(self.frame)
        self.Sono_AgitadoSim_2.setEnabled(True)
        self.Sono_AgitadoSim_2.setGeometry(QtCore.QRect(240, 200, 82, 17))
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(10)
        self.Sono_AgitadoSim_2.setFont(font)
        self.Sono_AgitadoSim_2.setStyleSheet("color: rgb(255, 255, 255);")
        self.Sono_AgitadoSim_2.setAutoRepeat(True)
        self.Sono_AgitadoSim_2.setAutoExclusive(False)
        self.Sono_AgitadoSim_2.setObjectName("Sono_AgitadoSim_2")
        self.Sono_AgitadoNao_2 = QtWidgets.QRadioButton(self.frame)
        self.Sono_AgitadoNao_2.setEnabled(True)
        self.Sono_AgitadoNao_2.setGeometry(QtCore.QRect(240, 220, 82, 17))
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(10)
        self.Sono_AgitadoNao_2.setFont(font)
        self.Sono_AgitadoNao_2.setStyleSheet("color: rgb(255, 255, 255);")
        self.Sono_AgitadoNao_2.setAutoRepeat(True)
        self.Sono_AgitadoNao_2.setAutoExclusive(False)
        self.Sono_AgitadoNao_2.setObjectName("Sono_AgitadoNao_2")
        self.Drogalicita_SIM_2 = QtWidgets.QRadioButton(self.frame)
        self.Drogalicita_SIM_2.setEnabled(True)
        self.Drogalicita_SIM_2.setGeometry(QtCore.QRect(420, 200, 82, 17))
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(10)
        self.Drogalicita_SIM_2.setFont(font)
        self.Drogalicita_SIM_2.setStyleSheet("color: rgb(255, 255, 255);\n"
"")
        self.Drogalicita_SIM_2.setAutoRepeat(True)
        self.Drogalicita_SIM_2.setAutoExclusive(False)
        self.Drogalicita_SIM_2.setObjectName("Drogalicita_SIM_2")
        self.Drogalicita_NAO_2 = QtWidgets.QRadioButton(self.frame)
        self.Drogalicita_NAO_2.setEnabled(True)
        self.Drogalicita_NAO_2.setGeometry(QtCore.QRect(420, 220, 82, 17))
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(10)
        self.Drogalicita_NAO_2.setFont(font)
        self.Drogalicita_NAO_2.setStyleSheet("color: rgb(255, 255, 255);")
        self.Drogalicita_NAO_2.setAutoRepeat(True)
        self.Drogalicita_NAO_2.setAutoExclusive(False)
        self.Drogalicita_NAO_2.setObjectName("Drogalicita_NAO_2")
        self.txt_senha = QtWidgets.QLineEdit(self.frame)
        self.txt_senha.setGeometry(QtCore.QRect(250, 420, 133, 20))
        self.txt_senha.setStyleSheet("background-color: rgb(34, 150, 213);\n"
"border:solid;\n"
"border-radius:30px;\n"
"color:white;")
        self.txt_senha.setInputMask("")
        self.txt_senha.setText("")
        self.txt_senha.setEchoMode(QtWidgets.QLineEdit.Password)
        self.txt_senha.setObjectName("txt_senha")
        self.lbl_nome_8 = QtWidgets.QLabel(self.frame)
        self.lbl_nome_8.setGeometry(QtCore.QRect(250, 390, 371, 19))
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(12)
        font.setBold(True)
        font.setWeight(75)
        self.lbl_nome_8.setFont(font)
        self.lbl_nome_8.setStyleSheet("color: rgb(255, 255, 255);")
        self.lbl_nome_8.setObjectName("lbl_nome_8")
        self.lbl_Admin = QtWidgets.QLabel(self.frame)
        self.lbl_Admin.setGeometry(QtCore.QRect(70, 390, 121, 16))
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(12)
        font.setBold(True)
        font.setWeight(75)
        self.lbl_Admin.setFont(font)
        self.lbl_Admin.setStyleSheet("color:white")
        self.lbl_Admin.setObjectName("lbl_Admin")
        self.Adm_SIM = QtWidgets.QRadioButton(self.frame)
        self.Adm_SIM.setGeometry(QtCore.QRect(80, 420, 82, 17))
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(10)
        self.Adm_SIM.setFont(font)
        self.Adm_SIM.setStyleSheet("color:white")
        self.Adm_SIM.setObjectName("Adm_SIM")
        self.Adm_NAO = QtWidgets.QRadioButton(self.frame)
        self.Adm_NAO.setGeometry(QtCore.QRect(80, 440, 82, 17))
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(10)
        self.Adm_NAO.setFont(font)
        self.Adm_NAO.setStyleSheet("color:white")
        self.Adm_NAO.setObjectName("Adm_NAO")
        self.layoutWidget = QtWidgets.QWidget(self.centralwidget)
        self.layoutWidget.setGeometry(QtCore.QRect(140, 230, 84, 48))
        self.layoutWidget.setObjectName("layoutWidget")
        self.verticalLayout = QtWidgets.QVBoxLayout(self.layoutWidget)
        self.verticalLayout.setContentsMargins(0, 0, 0, 0)
        self.verticalLayout.setObjectName("verticalLayout")
        self.Sexo_masculino = QtWidgets.QRadioButton(self.layoutWidget)
        self.Sexo_masculino.setEnabled(True)
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(10)
        self.Sexo_masculino.setFont(font)
        self.Sexo_masculino.setStyleSheet("color: rgb(255, 255, 255);")
        self.Sexo_masculino.setAutoRepeat(True)
        self.Sexo_masculino.setAutoExclusive(False)
        self.Sexo_masculino.setObjectName("Sexo_masculino")
        self.verticalLayout.addWidget(self.Sexo_masculino)
        self.Sexo_Feminino = QtWidgets.QRadioButton(self.layoutWidget)
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(10)
        font.setBold(False)
        font.setWeight(50)
        self.Sexo_Feminino.setFont(font)
        self.Sexo_Feminino.setStyleSheet("color: rgb(255, 255, 255);")
        self.Sexo_Feminino.setChecked(False)
        self.Sexo_Feminino.setAutoRepeat(True)
        self.Sexo_Feminino.setAutoExclusive(False)
        self.Sexo_Feminino.setObjectName("Sexo_Feminino")
        self.verticalLayout.addWidget(self.Sexo_Feminino)
        self.layoutWidget1 = QtWidgets.QWidget(self.centralwidget)
        self.layoutWidget1.setGeometry(QtCore.QRect(310, 230, 48, 48))
        self.layoutWidget1.setObjectName("layoutWidget1")
        self.verticalLayout_2 = QtWidgets.QVBoxLayout(self.layoutWidget1)
        self.verticalLayout_2.setContentsMargins(0, 0, 0, 0)
        self.verticalLayout_2.setObjectName("verticalLayout_2")
        self.Sono_AgitadoSim = QtWidgets.QRadioButton(self.layoutWidget1)
        self.Sono_AgitadoSim.setEnabled(True)
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(10)
        self.Sono_AgitadoSim.setFont(font)
        self.Sono_AgitadoSim.setStyleSheet("color: rgb(255, 255, 255);")
        self.Sono_AgitadoSim.setAutoRepeat(True)
        self.Sono_AgitadoSim.setAutoExclusive(False)
        self.Sono_AgitadoSim.setObjectName("Sono_AgitadoSim")
        self.verticalLayout_2.addWidget(self.Sono_AgitadoSim)
        self.Sono_AgitadoNao = QtWidgets.QRadioButton(self.layoutWidget1)
        self.Sono_AgitadoNao.setEnabled(True)
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(10)
        self.Sono_AgitadoNao.setFont(font)
        self.Sono_AgitadoNao.setStyleSheet("color: rgb(255, 255, 255);")
        self.Sono_AgitadoNao.setAutoRepeat(True)
        self.Sono_AgitadoNao.setAutoExclusive(False)
        self.Sono_AgitadoNao.setObjectName("Sono_AgitadoNao")
        self.verticalLayout_2.addWidget(self.Sono_AgitadoNao)
        self.layoutWidget2 = QtWidgets.QWidget(self.centralwidget)
        self.layoutWidget2.setGeometry(QtCore.QRect(490, 230, 48, 48))
        self.layoutWidget2.setObjectName("layoutWidget2")
        self.verticalLayout_3 = QtWidgets.QVBoxLayout(self.layoutWidget2)
        self.verticalLayout_3.setContentsMargins(0, 0, 0, 0)
        self.verticalLayout_3.setObjectName("verticalLayout_3")
        self.Drogalicita_SIM = QtWidgets.QRadioButton(self.layoutWidget2)
        self.Drogalicita_SIM.setEnabled(True)
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(10)
        self.Drogalicita_SIM.setFont(font)
        self.Drogalicita_SIM.setStyleSheet("color: rgb(255, 255, 255);\n"
"")
        self.Drogalicita_SIM.setAutoRepeat(True)
        self.Drogalicita_SIM.setAutoExclusive(False)
        self.Drogalicita_SIM.setObjectName("Drogalicita_SIM")
        self.verticalLayout_3.addWidget(self.Drogalicita_SIM)
        self.Drogalicita_NAO = QtWidgets.QRadioButton(self.layoutWidget2)
        self.Drogalicita_NAO.setEnabled(True)
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(10)
        self.Drogalicita_NAO.setFont(font)
        self.Drogalicita_NAO.setStyleSheet("color: rgb(255, 255, 255);")
        self.Drogalicita_NAO.setAutoRepeat(True)
        self.Drogalicita_NAO.setAutoExclusive(False)
        self.Drogalicita_NAO.setObjectName("Drogalicita_NAO")
        self.verticalLayout_3.addWidget(self.Drogalicita_NAO)
        self.layoutWidget.raise_()
        self.layoutWidget.raise_()
        self.layoutWidget.raise_()
        self.frame.raise_()
        self.btn_goCadastro2.raise_()
        self.label.raise_()
        self.lbl_nome_2.raise_()
        self.txt_cpf.raise_()
        self.lbl_nome.raise_()
        self.txt_nome.raise_()
        self.lbl_idade.raise_()
        self.txt_idade.raise_()
        self.lbl_nome_3.raise_()
        self.lbl_nome_4.raise_()
        self.spinHorasDormidas.raise_()
        self.lbl_nome_5.raise_()
        self.spinXicarasCafe.raise_()
        self.lbl_nome_6.raise_()
        self.spinExercicioSemana.raise_()
        self.lbl_nome_7.raise_()
        self.spinExercicioSemana_2.raise_()
        self.lbl_nome_9.raise_()
        self.lbl_nome_10.raise_()
        editar_Adm.setCentralWidget(self.centralwidget)

        self.retranslateUi(editar_Adm)
        QtCore.QMetaObject.connectSlotsByName(editar_Adm)

    def retranslateUi(self, editar_Adm):
        _translate = QtCore.QCoreApplication.translate
        editar_Adm.setWindowTitle(_translate("editar_Adm", "ZSQADM - Editar Usuário"))
        self.btn_goCadastro2.setText(_translate("editar_Adm", "Avançar"))
        self.label.setText(_translate("editar_Adm", "Dados Pessoais"))
        self.lbl_nome_2.setText(_translate("editar_Adm", "CPF"))
        self.lbl_nome.setText(_translate("editar_Adm", "Nome"))
        self.lbl_idade.setText(_translate("editar_Adm", "Idade"))
        self.lbl_nome_3.setText(_translate("editar_Adm", "Qual o seu sexo?"))
        self.lbl_nome_4.setText(_translate("editar_Adm", "Quantas horas você dorme por dia:"))
        self.lbl_nome_5.setText(_translate("editar_Adm", "Quantas xícaras de café você toma por dia?"))
        self.lbl_nome_6.setText(_translate("editar_Adm", "Quantidade de dias que se exercita por semana:"))
        self.lbl_nome_7.setText(_translate("editar_Adm", "De 0 a 10 quanto stress você passa:"))
        self.lbl_nome_9.setText(_translate("editar_Adm", "Seu sono é agitado?"))
        self.lbl_nome_10.setText(_translate("editar_Adm", "Você usa drogas lícitas?"))
        self.Sexo_Feminino_2.setText(_translate("editar_Adm", "Feminino"))
        self.Sexo_masculino_2.setText(_translate("editar_Adm", "Masculino"))
        self.Sono_AgitadoSim_2.setText(_translate("editar_Adm", "Sim"))
        self.Sono_AgitadoNao_2.setText(_translate("editar_Adm", "Não"))
        self.Drogalicita_SIM_2.setText(_translate("editar_Adm", "Sim"))
        self.Drogalicita_NAO_2.setText(_translate("editar_Adm", "Não"))
        self.lbl_nome_8.setText(_translate("editar_Adm", "Senha"))
        self.lbl_Admin.setText(_translate("editar_Adm", "Administrador?"))
        self.Adm_SIM.setText(_translate("editar_Adm", "Sim"))
        self.Adm_NAO.setText(_translate("editar_Adm", "Não"))
        self.Sexo_masculino.setText(_translate("editar_Adm", "Masculino"))
        self.Sexo_Feminino.setText(_translate("editar_Adm", "Feminino"))
        self.Sono_AgitadoSim.setText(_translate("editar_Adm", "Sim"))
        self.Sono_AgitadoNao.setText(_translate("editar_Adm", "Não"))
        self.Drogalicita_SIM.setText(_translate("editar_Adm", "Sim"))
        self.Drogalicita_NAO.setText(_translate("editar_Adm", "Não"))
