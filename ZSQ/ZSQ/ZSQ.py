...
class editar_Adm(QMainWindow,Ui_editar_Adm):
      ...
      def guarda_dado(self):
           ...
           dataset = pd.read_csv('/home/pi/Documents/projeto tcc/ZSQ/ZSQ/registrosTreinamentoIa.csv')
           ...
           prob_antigo=self.usuario_edt.prob

           usuario_edt.prob = problema_final
           
           cpf_pravalidar1=usuario_edt.cpf
           cpf_pravalidar1=cpf_pravalidar1.replace('-','')
           cpf_pravalidar1=cpf_pravalidar1.replace('.','')

           if Connector.valida_cpf(cpf_pravalidar1):
               if Connector.check_cpf(self,usuario_edt.cpf):
                   Connector.editUsuario_ADM(self,usuario_edt,cpf_antigo)

                   if prob_antigo!=problema_final:
                        if Connector.check_solution(self,usuario_edt.id,usuario_edt.prob):
                            solucao= Solucao(problema_final,0,usuario_edt.id,0,0,0,0) 
...
class Tela_logado(QMainWindow,Ui_Tela_Logado):
    ...
    def pega_usuario(self,usuarioAtual):
        dataset = pd.read_csv('/home/pi/Documents/projeto tcc/ZSQ/ZSQ/testesMelhorResultadoIa.csv')
